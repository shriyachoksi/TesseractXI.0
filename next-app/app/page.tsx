...
export default function Index() {
  ...
  const events = [
    { title: "AI Summit 2025", img: "https://images.pexels.com/photos/7234409/pexels-photo-7234409.jpeg", desc: "Talks and panels on AI safety, ML systems, and ethics.", tag: "Upcoming" },
    { title: "Hack the Future", img: "https://images.pexels.com/photos/1181260/pexels-photo-1181260.jpeg", desc: "36-hour hackathon focused on sustainability and edge compute.", tag: "Past" },
    { title: "Robotics Workshop", img: "https://images.pexels.com/photos/7869041/pexels-photo-7869041.jpeg", desc: "Hands-on build with microcontrollers, actuators, and CV.", tag: "Past" },
  ];
  ...
  const team = [
    ...
    { name: "Ethan Lee", role: "Program Manager", img: "https://images.pexels.com/photos/5474038/pexels-photo-5474038.jpeg" },
    { name: "Sofia Reyes", role: "Community Lead", img: "https://images.pexels.com/photos/33728824/pexels-photo-33728824.jpeg" },
  ];
  ...
  return (
    <div>
      <section className="relative h-[72vh] min-h-[500px] w-full overflow-hidden">
        <video className="pointer-events-none absolute inset-0 h-full w-full object-cover" autoPlay muted loop playsInline poster="https://images.pexels.com/photos/8720586/pexels-photo-8720586.jpeg">
        ...
        <div className="relative z-10 container h-full flex flex-col items-center justify-center text-center">
          <h1 className="font-title uppercase max-w-4xl text-3xl md:text-5xl font-extrabold leading-tight tracking-wider text-white drop-shadow-2xl">Tesseract 2025</h1>
          <p className="font-tech mt-2 max-w-3xl text-xl md:text-2xl text-white/90 drop-shadow">Where technology meets creativity</p>
          <div className="mt-8"><a href="#events"><Button size="pill" className="font-tech shadow-md hover:shadow-lg">Explore More</Button></a></div>
        </div>
      </section>
      ...
      <section id="events" className="container py-16 md:py-24">
        ...
        <div className="mt-10 text-center"><a href="#events"><Button size="pill" className="font-tech shadow-md hover:shadow-lg">View All Events</Button></a></div>
      </section>
      ...
      <section id="team" className="container py-16 md:py-24">
        ...
        <div className="mt-8 text-center"><a href="#team"><Button size="pill" className="font-tech shadow-md hover:shadow-lg">View All Members</Button></a></div>
      </section>
      ...
      <section id="gallery" className="container py-16 md:py-24">
        ...
        <div className="mt-10 -mx-8"><DraggableCardDemo /></div>
        <div className="mt-8 text-center"><a href="#gallery"><Button size="pill" className="font-tech shadow-md hover:shadow-lg">View Gallery</Button></a></div>
      </section>
      ...
      <section id="sponsors" className="container py-16 md:py-24">
        ...
        <div className="mt-8 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {["AlphaTech","Nova Labs","QuantumWorks","ByteForge","Orbit AI","Vertex Cloud"].map((brand, i) => (
            <div key={brand} className="group">
              <div className="relative rounded-2xl p-[2px] bg-gradient-to-br from-primary/40 via-accent/20 to-transparent">
                <div className="relative grid place-items-center rounded-2xl bg-card/70 backdrop-blur-sm px-6 py-7 shadow-sm ring-1 ring-border transition-all group-hover:ring-primary/40">
                  ...
                  <div className="mt-3 flex items-center gap-2"><span className="inline-flex items-center rounded-full bg-secondary/60 px-2.5 py-1 text-[10px] font-semibold text-foreground/80 ring-1 ring-border">Partner</span></div>
                </div>
              </div>
            </div>
          ))}
      ...
      <section id="register" className="container pb-20">
        <div className="text-center mb-6"><h2 className="text-3xl md:text-4xl font-bold">Join the Revolution</h2></div>
        <Countdown targetDate={new Date(new Date().getFullYear(), 9, 1)} />
      </section>

      <Dialog open={!!lightboxSrc} onOpenChange={(o) => !o && setLightboxSrc(null)}>
        <DialogContent className="max-w-4xl">
          {lightboxSrc && (<img src={lightboxSrc} alt="Gallery large view" className="w-full h-auto rounded-lg" />)}
        </DialogContent>
      </Dialog>
    </div>
  );
...
function FocusCards({ items }: { items: { title: string; img: string; desc: string; tag: string }[] }) {
  ...
  return (
    <div className="mt-10 grid grid-cols-1 md:grid-cols-3 gap-6 md:[perspective:1200px]" onMouseLeave={() => setHovered(null)}>
      {items.map((item, i) => (
        <FocusCard key={item.title} item={item} active={hovered === i} dimmed={hovered !== null && hovered !== i} onEnter={() => setHovered(i)} />
      ))}
    </div>
  );
}

function FocusCard({ item, active, dimmed, onEnter }: { item: { title: string; img: string; desc: string; tag: string }; active: boolean; dimmed: boolean; onEnter: () => void; }) {
  const [mx, setMx] = useState(0);
  const [my, setMy] = useState(0);
  return (
    <article onMouseEnter={onEnter} onMouseMove={(e) => { const r = (e.currentTarget as HTMLDivElement).getBoundingClientRect(); setMx(e.clientX - r.left); setMy(e.clientY - r.top); }} className={cn("group relative rounded-2xl border bg-card overflow-hidden transition-all duration-300", active && "md:scale-[1.04] md:shadow-xl md:ring-2 md:ring-primary/40", dimmed && "md:opacity-60 md:[filter:blur(1px)] md:scale-[0.98]") }>
      <div className="relative">
        <img src={item.img} alt={item.title} loading="lazy" className="h-48 w-full object-cover"/>
        <span className="absolute left-4 top-4 rounded-full bg-background/80 px-3 py-1 text-xs font-medium text-primary border">{item.tag}</span>
        <div className="pointer-events-none absolute inset-0 opacity-0 md:group-hover:opacity-100 transition-opacity" style={{ background: `radial-gradient(360px circle at ${mx}px ${my}px, hsl(var(--ring)/0.25), transparent 40%)` }} />
      </div>
      <div className="p-5"><h3 className="text-lg font-semibold text-primary">{item.title}</h3><p className="mt-2 text-sm text-foreground/70">{item.desc}</p></div>
    </article>
  );
}
...
function SectionHeader({ title, subtitle }: { title: string; subtitle: string }) {
  return (
    <div className="max-w-2xl"><h2 className="font-tech text-3xl md:text-4xl font-bold">{title}</h2><p className="mt-2 text-foreground/70">{subtitle}</p></div>
  );
}
    ...
    <div className="flex flex-col items-center gap-3">
      <div className="relative">
        ...
        <Avatar className="relative h-16 w-16 ring-1 ring-border group-hover:ring-primary/40">
          ...
          <AvatarFallback className="text-sm font-semibold" style={{ background: c, color: "#1a1a1a" }}>{initials}</AvatarFallback>
        </Avatar>
      </div>
