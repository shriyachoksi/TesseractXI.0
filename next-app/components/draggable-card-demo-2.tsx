...
export default function DraggableCardDemo() {
  ...
  const items = React.useMemo(() => Array.from({ length: 46 }, (_, i) => ({ title: `Gallery Shot ${i + 1}`, image: pool[i % pool.length] })), [pool.length]);
  ...
  const placements = React.useMemo(() => {
    const rand = mulberry32(123456); // deterministic seed for SSR/CSR match
    const sizes = [ { w: 280, h: 360 }, { w: 230, h: 310 }, { w: 190, h: 250 } ];
    return items.map((_, i) => {
      const size = sizes[i % sizes.length];
      const sizeJitter = 1 + (rand() * 0.25 - 0.125);
      return {
        top: 5 + rand() * 90,
        left: 5 + rand() * 90,
        rotate: -12 + rand() * 24,
        z: Math.floor(rand() * 12),
        w: size.w * sizeJitter,
        h: size.h * sizeJitter,
      };
    });
  ...
  return (
    <DraggableCardContainer className="relative flex min-h-[130vh] w-full items-center justify-center overflow-hidden">
      {items.map((item, i) => (
        <DraggableCardBody
          key={`${item.title}-${i}`}
          className="absolute"
          style={{
            ...
            rotate: `${placements[i].rotate}deg`,
            zIndex: 10 + placements[i].z,
            width: placements[i].w,
            height: placements[i].h,
          }}
        >
          <img src={item.image} alt={item.title} className="pointer-events-none relative z-10 h-full w-full object-cover shadow-lg" />
        </DraggableCardBody>
      ))}
